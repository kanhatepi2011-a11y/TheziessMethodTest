...
export default async function handler(req, res) {
  ...
  if (!clientId || !redirectUri) {
    return res.status(500).json({
      error: "TELEGRAM_CLIENT_ID or TELEGRAM_REDIRECT_URI is missing",
    });
  }
  ...
  res.setHeader("Set-Cookie", [
    `telegram_oauth_state=${state}; Path=/; HttpOnly; SameSite=Lax; Max-Age=600${secure}`,
    `telegram_pkce_verifier=${verifier}; Path=/; HttpOnly; SameSite=Lax; Max-Age=600${secure}`,
  ]);
